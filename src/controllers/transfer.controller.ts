...
export const getTransfers = async (req: Request, res: Response) => {
    try {
        ...
        const coinbaseWalletId = (user.wallet as any).coinbaseWalletId as string | undefined;

        if (coinbaseWalletId) {
            wallet = await Wallet.fetch(coinbaseWalletId);
        } else {
            // Fallback: no coinbaseWalletId stored — look up wallet by address
            console.warn('[api/transfer/get] coinbaseWalletId missing — attempting to locate wallet by address:', user.wallet.address);

            const walletsList = await Wallet.listWallets();
            for (const w of walletsList.data) {
                try {
                    const addrs = await w.listAddresses();
                    const found = addrs.find((a: any) => a.getId() === user.wallet.address);
                    if (found) {
                        wallet = w;
                        break;
                    }
        ...
        if (!wallet) {
            console.error('[api/transfer/get] Coinbase wallet not found for user wallet', user.wallet);
                    ...
                    destinationUser = {
                        wallet: {
                            rewards: {
                                ...
                                lastUpdated: foundUser.wallet.rewards?.lastUpdated.toISOString() || new Date().toISOString(),
...
export const createTransfer = async (req: Request, res: Response) => {
    try {
        ...
        const { asset, data } = req.body as { asset?: string; data?: { recipient?: string; amount?: number } };
        ...
        const user = await prisma.user.findFirst({ where: { userId: token }, include: { wallet: { include: { rewards: true } }, faucet: true } }) as any;
        if (!user) {
            return res.status(404).json({ error: "User not found" });
        }
        ...
        if (coinbaseWalletId) {
            ...
            console.warn('[api/transfer/create] coinbaseWalletId missing — attempting to locate wallet by address:', user.wallet.address);
            const walletsList = await Wallet.listWallets();
            for (const w of walletsList.data) {
                try {
                    const addrs = await w.listAddresses();
        ...
        if (!wallet) {
            console.error('[api/transfer/create] Coinbase wallet not found for user', { userId: user.userId, wallet: user.wallet });
            return res.status(404).json({ error: 'Coinbase wallet not found' });
        }
        ...
        const destUser = await prisma.user.findUnique({ where: { email: recipient }, include: { wallet: true } }) as any;
        const destinationAddress = destUser?.wallet?.address ? destUser.wallet.address : recipient;

        // Create transfer and wait
        const transferOp = await (await wallet.createTransfer({
            amount: amount,
            assetId: asset,
        ...
        try {
            ...
            return res.json({ transactionLink: transferResult.getTransactionLink(), status: transferResult.getStatus() });
        } catch (err) {
